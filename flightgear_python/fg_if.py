...
class FGConnection:
    ...
    def connect_rx(self, fg_host: str, fg_port: int, rx_cb: rx_callback_type) -> EventPipe:
        ...
        try:
            ...
            if self._disconnect_callback:
                    self._disconnect_callback(True)
            exit()
    ...
    def set_disconnect_callback(self, disconnect_callback: callable):
        """
        Set up a callback that will be called when the connection is lost

        :param disconnect_callback: callable function
        """
    ...
    def _fg_packet_roundtrip(self):
        ...
        try:
            ...
            if self._disconnect_callback:
                    self._disconnect_callback(True)
            exit()
        except FGCommunicationError as e:
            print(e)
            exit()
    ...
    def start(self):
        ...
        """
        self.rx_proc = mp.Process(target=self._rx_process, daemon=True)
        self.rx_proc.start()
